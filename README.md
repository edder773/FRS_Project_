...
> 커뮤니티
```
1. 자유롭게 토론할 수 있는 자유 게시판 기능 제공 (작성, 수정, 삭제)
2. 게시판 별 댓글 기능 제공 (작성, 수정, 삭제)
3. 자주 묻는 질문 게시판 제공
4. 1:1 문의 게시판 제공
```
<br>
...
2. 카카오맵을 이용한 위치 선택 가능
3. 마커를 클릭하면 해당 은행 홈페이지로 이동 가능
```
<br>
...
### 1-3 컴포넌트 구조
![component](https://github.com/edder773/FRS_Project_/assets/58801719/1405ac84-597c-45d6-bb66-57c2a98e1a96)
...
> - 다른 사용자가 작성한 글을 확인 가능하고, 자신의 글만 수정 및 삭제 가능
> - 게시된 글에 대해 댓글 기능 제공, 자신의 댓글만 수정 및 삭제 가능
...
> - 현재 가입한 본인 정보 제공
> - 수정하기 기능을 통해 선택사항 데이터(나이, 자산 등) 수정 가능
> - 내가 가입한 상품 리스트 제공
...
> API를 가져오는 동안 다른 함수가 먼저 동작해서 발생한 오류로 해결하기 위해 비동기적으로 구현하였고, 필터링도 은행, 가입 기간을 기준으로 분류할 수 있게 구현하였다.
...
> 페이지 디자인은 잘 구현되어 있는 여러 홈페이지를 참고해봤지만 공식 문서와 구글링(+chatgpt)이 가장 도움이 된 거 같다. <br>
> 시간 배분을 나름 잘 하면서 진행한다고 생각했는데, 너무 부족해서 나중에는 급하게 구현하느라 고생했다. <br>
> FRS 프로젝트 기간이 끝나더라도 아쉽게 해결하지 못한 남은 작업(모달 내의 지도, 가입한 상품의 차트, 1:1문의 게시판)들을 모두 해결하고 싶다.<br>
> 프로젝트 기간동안 힘들었지만, 함께한 보석님의 도움을 받으며 프로젝트에 임할 수 있어 감사했다. <br>
