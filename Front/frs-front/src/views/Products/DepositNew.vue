<template>
    ...
    <b-modal v-model="recommendItem" @hide="closeRecommend">
      ...
      <div class="filter-container">
        ...
        <b-table striped hover :items="filteredProducts" :fields="tableFields">
            <template #cell(id)="row">
        <button @click="openModal(row.item)">{{ row.value }}</button>
        </template>
        </b-table>
  ...
  export default {
    ...
    methods: {
      fetchProducts() {
        ...
        axios
          .get('http://127.0.0.1:8000/deposits/products/')
          .then((response) => {
